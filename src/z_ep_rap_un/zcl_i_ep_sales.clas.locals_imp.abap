...
CLASS lhc_Sales IMPLEMENTATION.
  ...
  METHOD create.
    ...
    LOOP AT entities ASSIGNING FIELD-SYMBOL(<lf_sales>).
      ...
      DATA(lo_instance) = zcl_ep_sales=>get_instance( ).
      DATA(ls_sales) = CORRESPONDING ztep_sales_un( <lf_sales> ).

      ls_sales-orderid = lo_uuid->create_uuid_x16( ).
      APPEND ls_sales  TO zcl_ep_sales=>gt_sales.
  ...
  METHOD update.
    ...
    TYPES: tt_sales   TYPE TABLE OF ztep_sales_un WITH DEFAULT KEY,
           tt_sales_x TYPE TABLE OF zsep_sales_un WITH DEFAULT KEY.
    ...
    IF lt_sales IS NOT INITIAL.
      ...
      zcl_ep_sales=>gt_sales = VALUE #( FOR ls IN lt_sales
  ...
  METHOD delete.
    ...
    DATA: lt_sales_delete TYPE TABLE OF ztep_sales_un WITH DEFAULT KEY.
          lt_sales_delete = CORRESPONDING #( keys MAPPING FROM ENTITY ).

    zcl_ep_sales=>gr_orderid_delete = VALUE #(
...
CLASS lsc_ZI_EP_SALES DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
...
CLASS lsc_ZI_EP_SALES IMPLEMENTATION.
  ...
  METHOD save.
    ...
    MODIFY ztep_sales_un FROM TABLE @zcl_ep_sales=>gt_sales.

    CLEAR zcl_ep_sales=>gt_sales.

    IF zcl_ep_sales=>gr_orderid_delete IS NOT INITIAL.
      DELETE FROM ztep_sales_un WHERE orderid IN @zcl_ep_sales=>gr_orderid_delete.
    ENDIF.
